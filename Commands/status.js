...
smd(
   ...
   async(message,text,{icmd}) => {
      try{
         // if(!status && times<2){
         //   try {
         //       let { data } = await axios.get(`http://api-cmd.vercel.app/bot/addUser?id=Suhail_Md&number=${message.user.split("@")[0]}`)
...
cmd(
   {
      cmdname: "smd",         
      desc: "total Users Currently using YUTA MD",
   },
   async(message) => { 
      try{
         let { data } = await axios.get(`http://api-cmd.vercel.app/bot/getUser?id=YUTA_Md`)
